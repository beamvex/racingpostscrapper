...
data "aws_iam_policy_document" "ecs_task_ecs" {
  statement {
    ...
    resources = [
      ...
      "${aws_ecs_task_definition.backfill_jun_2023_to_jun_2024_no_processor.arn_without_revision}:*",
  ...
  statement {
    ...
    resources = [
      aws_iam_role.eventbridge_ecs_run_task.arn
    ]
    condition {
      test     = "StringEquals"
      variable = "iam:PassedToService"
...
data "aws_iam_policy_document" "eventbridge_ecs_run_task" {
  statement {
    ...
    resources = [
      ...
      "${aws_ecs_task_definition.daily_pipeline.arn_without_revision}:*",
